"""
Client implementation for Multi-GPU Scheduler
"""
...
from ..utils.network import connect_to_server, send_json_message, receive_json_message
...
class SimpleClient:
    ...
    def submit_job(self, gpus: int, cmd: str, interactive: bool = False, 
        ...
        try:
            ...
            # Connect to server
            sock = connect_to_server(self.host, self.port, timeout_config['connection_timeout'])
            if not sock:
                print(f"Failed to connect to server at {self.host}:{self.port}")
                return False
            
            # Send request
            if not send_json_message(sock, request, timeout_config['connection_timeout']):
    ...
    def _handle_interactive_session(self, sock: socket.socket, timeout_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            response = receive_json_message(sock, timeout_config['connection_timeout'])
            if not response:
    ...
    def get_queue_status(self) -> bool:
        ...
        try:
            ...
            sock = connect_to_server(self.host, self.port, 10.0)
            if not sock:
                print(f"Failed to connect to server at {self.host}:{self.port}")
                return False
